...
<div id="contact" class="main_box">
    ...
    <?php
        ...
        if (isset($_POST["submit"]))
        {
            ...
            $sql = "INSERT INTO tours (name, image) values('".$name."', '".$_FILES["image"]['name']."')";
            $result = mysqli_query($connection, $sql);

          header("location: admin.php");
        }
    ?>
    <form method="post" name="contact" action="admin.php" enctype="multipart/form-data">
        <label for="author">Tour Name:</label> 
        <input type="text" id="author" name="name" class="required input_field" />
        <div class="cleaner h10"></div>
                                    
        <label for="email">Tour Image:</label> 
        <input type="file" name="image" class="validate-email required input_field">
        <div class="cleaner h10"></div>
                                
        <input type="submit" value="Add" id="submit" name="submit" class="submit_btn float_l" />
